...
async function modeF(): Promise<void> {
  await check('F1', 'F', 'enrichment: живой Vertex draft (профиль без своих ссылок)', '200 + draft/sources', async () => {
    ...
    return {
      ...
      deviation:
        'живая ручка отвечает 502 enrichment_failed (retryable) за ~6.6с для профиля без собственных ссылок: провайдер возвращает code=no_draft (ответ модели не парсится в черновик). Тот же ключ/модель локально дают draft, когда у профиля есть хотя бы одна ссылка (F1b), т.е. это хрупкость промпта/парсинга на «бедных» профилях, а не проблема конфигурации.',
    };
  });
