...
## Address-book import without Google OAuth — 2026-09-15 (increment)

Counters (was → now): **unit 438 → 459, integration 322 → 330 (+1 skip), e2e 24 → 26**; all 7 gates exit 0 (`pnpm gates` → `evidence/final-gates.json`, previous run kept in `previous_run`); live usage matrix **123 → 126 checks** (122/0/1 → **125 PASS / 0 FAIL / 1 SKIP**).

- **"Who of my contacts is already here" — today, with no OAuth client.** `POST /api/me/contacts/import` reads a vCard (.vcf) or CSV address book, parses it in memory (`src/domain/contact-import.ts`, pure: FN/N/EMAIL, folded lines, CRLF, group prefixes; base64/quoted-printable payloads and PHOTO/LOGO/KEY/SOUND are ignored rather than half-decoded; CSV column auto-detection EN/RU + positional fallback; 5 MB / 5000 contacts, over-limit is reported, never silently trimmed) and matches every address through the existing peppered key (`emailLookupHash` + `HASH_PEPPER`) against `accounts.email_lookup_hash` (active accounts, excluding the caller). The answer is display names, slugs and headlines — never an address. UI: "Import your contacts" card on `/me/connections` (file or paste, EN/RU/ES).
- **Nothing is stored.** Table snapshot around the live import: 18 storage tables, delta 0; the imported address exists as a lookup key nowhere (0 rows); the only DB write is one append-only `audit_events` fact holding two numbers (`scanned`, `matched_count`). The 5/hour/account budget lives in memory (`consumeSubjectToken`, ADR 0005 semantics: 5 immediately, then one token every 12 minutes; 6th → 429 + `Retry-After`) precisely so the endpoint leaves no rate-limit row either.
- **Registry + doc:** `match` capability on the `vcard`/`csv` rows (§A2 deviation 5, declared in the header) and `docs-internal/product/GOOGLE_OAUTH_SETUP.md` — the owner's step-by-step for the Phase 2 Google client, stating plainly that nothing is implemented behind it yet. The registry's `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` are canonical; the brief's `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` are the same secret under a second spelling (documented, one-line change if preferred).
- **Live checks added:** S9 (the panel is on the page), S11 (a fixture neighbour is found by the address in a .vcf; no `@` in the response), S12 (the table snapshot), S13 (401 without a session; 6th request → 429 while another account still gets 200).
- **Not pushed, not deployed.** 5 commits on `main` in `welcome/`: `ea74261` (parser + API), `edcfcfa` (UI + i18n), `08cf21a` (registry + docs), `14ac172` (tests), and the matrix/evidence commit. Prod DB left as found: 0 MATRIX fixtures; the run's own `audit_events` facts (8) are append-only and stay, as designed.
