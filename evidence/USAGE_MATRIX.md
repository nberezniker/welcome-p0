...
- **Прогон:** 2026-09-23T22:13:44.074Z → 2026-09-23T22:17:40.481Z
...
| B6 | активные сессии: список без токенов, текущая помечена | 200; ровно ключи id/created_at/last_seen_at/current; одна current и она первая; в теле нет слова token | сессий=52, current-первая=true, ключи строк только id/created_at/last_seen_at/current, токенов в теле нет | **PASS** | {"id":"c1ae150d-7a29-41bb-8814-7cbabcf1c3c8","created_at":"2026-09-23T22:13:47.919Z","last_seen_at":"2026-09-23T22:13:47.919Z","current":true} |
...
| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| D1 | мини-лендинг GET /p/<slug>: 200 + имя/оси, без приватного телефона | 200, имя + метки интересов/интентов, не содержит телефон | 200; секции: interests«data-testid="pubcard-interests" aria-lab…», needs«data-testid="pubcard-needs" aria-labelle…», offers«data-testid="pubcard-offers" a…»; телефон отсутствует | **PASS** | {"interests":"data-testid=\"pubcard-interests\" aria-labelledby=\"pubcard-interests-title\"> Interests AI / ML Dev tools <section class=\"mt-6 border-t border-line pt-5\"","needs":"data-testid=\"pubca… |
...
| D4 | QR SVG: 200 image/svg+xml | 200 + <svg | 200 image/svg+xml | **PASS** | HTTP 200 image/svg+xml; charset=utf-8 bytes=2374 |
...
| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| I1 | invite → claim-URL для approved-регистрации | 200 + claim_url/expires_at | 200 + claim_url=/claim/«token» (7 дней) | **PASS** | HTTP 200 {"ok":true,"claim_url":"/claim/«token»","expires_at":"2026-09-30T22:16:58.768Z"} |
...
| J10 | уведомления о решении подавляются по правилам (no_channel / consent_revoked) | decline → suppressed:no_channel; withdraw при активном binding без service_channel-согласия → suppressed:consent_revoked | decline=suppressed:no_channel (нет канала), withdraw=suppressed:consent_revoked (binding активен, согласие отозвано) | **PASS** | delivery_attempts: intro_declined:1e73e8fb-4636-4a18-99f5-d0f6e58034f3:6480ab56-a189-4d01-8e0a-172e71e0e5c7 → suppressed/no_channel; intro_withdrawn:9e5d60e4-04aa-416a-8ada-2cccf31ef12b:186a2dd2-4e77-… |
| J11 | email-канал уведомлений: выбор канала и подавление | без привязки + email + согласие → email (не no_channel); с активной привязкой → telegram (email не используется); email без согласия → suppressed:consent_revoked; адрес не хранится в джобе | mailOnly=sent:email (провайдера у экземпляра нет — подавление честное, канал всё равно email, не no_channel); mailBound=failed:tg_http_400 (telegram приоритетнее); mailNoConsent=suppressed:consent_revoked | **PASS** | delivery_attempts: intro_requested:9f19e729-dc97-46af-819c-d89127f7ac2e:b22c5bcc-e858-49d3-8910-ca7d85b52c17 → sent:email; intro_requested:d4c53043-c0c0-42ee-bede-10dbe762a2f5:8ea0a13a-25d1-422f-b829-… |

## Режим K

| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| K1 | заметка владельцем: upsert + список | 200; заметка видна в своём списке | 200 + заметка в /api/me/notes | **PASS** | HTTP 200 {"ok":true,"note":{"other_profile_id":"«token»","note_text":"MATRIX- note about Delta","next_step":"ping","next_step_status":"proposed","updated_at":"2026-09-23T22:17:06.658Z"}} |
...
| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| P1 | health публичный: без migration_version | 200 status/db/worker и БЕЗ migration_version | 200 {"status":"ok","db":"up","worker":"up","worker_last_tick_age_seconds":1,"pending_jobs":0,"oldest_pending_job_age_seconds":null} (без migration_version) | **PASS** | HTTP 200 {"status":"ok","db":"up","worker":"up","«token»":1,"pending_jobs":0,"«token»":null} |
| P2 | health с x-health-details: отдаёт версию миграций | 200 + migration_version | 200 migrations=applied, migration_version=015 | **PASS** | HTTP 200 {"status":"ok","db":"up","migrations":"applied","migration_version":"015","worker":"up"} |
| P3 | security-заголовки на / и /login (5 заголовков) | CSP, X-Frame-Options, Referrer-Policy, Permissions-Policy, X-Content-Type-Options: nosniff | 5/5 на / и /login (content-security-policy, x-frame-options, referrer-policy, permissions-policy, x-content-type-options) | **PASS** | /: 5/5 \| /login: 5/5 |
| P4 | rate limit на OTP: per-IP bucket (10/мин) | 11-й запрос с одного IP → 429 + X-RateLimit-Limit | статусы 200,200,200,200,200,200,200,200,200,200,429 → 429 (X-RateLimit-Limit=10) | **PASS** | HTTP 429 {"code":"rate_limited","message":"Too many requests. Slow down and try again later.","correlation_id":"«token»","retryable":true} x-ratelimit-limit=10 |
...
| R2 | фактическая механика локали: cookie → локализованные строки | ru/es отдают свои строки, en — свои | en«business» / ru«вместо» / es«tarjetas» — все найдены на страницах | **PASS** | {"en":"business","ru":"вместо","es":"tarjetas"} |
...
| S5 | страница /e/<slug>: календарь и шэринг свёрнуты в контролы | есть .ics, есть контролы календаря и шэринга, а сырых диплинков в первом ответе нет | .ics + свёрнутые контролы календаря и шэринга, сырых диплинков нет | **PASS** | HTTP 200, 16205 bytes; раскрытие проверяют tests/e2e/event-actions.spec.ts |
...
| S9 | /me/connections: закрыт без сессии, с сессией показывает провайдеров без секретов | signed-out → redirect /login; signed-in → 200 + все id + только ИМЕНА переменных | signed-out 307 → /login; signed-in 200 с 15 карточками, панелью импорта и именами переменных | **PASS** | signed-out: 307 /login \| signed-in: HTTP 200, 103581 bytes |
| S10 | публичная карточка: og:url, «Добавить в контакты» и диплинки | og:url карточки, ссылка на vCard без сессии, 4 сети | 200: og:url + vCard + 4 сети, анонимная выгрузка .vcf работает, приватных целей нет | **PASS** | HTTP 200, 22060 bytes \| vCard HTTP 200 |
| S11 | POST /api/me/contacts/import: fixture-сосед находится по адресу из .vcf | 200, matched по slug, в теле нет ни одного email, 5/час | 200: scanned=2, matched=1 (MATRIX-Bravo / matrix-bravo-56ae038b0b102b5e), unmatched=1, ни одного email в ответе | **PASS** | HTTP 200 {"ok":true,"scanned":2,"matched_count":1,"unmatched_count":1} \| X-RateLimit-Limit=5 |
