...
- **Прогон:** 2026-09-15T10:15:17.507Z → 2026-09-15T10:18:52.044Z
- **Итог:** 122 PASS / 0 FAIL / 1 SKIP / 0 BLOCKED (всего 123)
- **Машинный отчёт:** [usage-matrix.json](usage-matrix.json)

## Инкремент этого прогона

Пакет «Фаза 1» задания `docs-internal/product/SOCIAL_INTEROP_AND_MATCHING.md`
(интероп-слой + matching v4). Семь пакетов, коммит после каждого.
...
| WP7 | Новые чеки матрицы + живой прогон против прод-БД | этот коммит | Режим S в scripts/usage-matrix.mts (10 чеков: providers, ICS, goals, режимы рекомендаций, connections, карточка), `createEvent` умеет расписание и комнатную ссылку, `updateProfile` переносит goals. | 123 чека: 122 PASS / 0 FAIL / 1 SKIP (было 113: 112/0/1). |
...
| test:unit | `pnpm test:unit` | **0** | 438 pass / 0 fail (было 366; +72: providers, goals, ics, share, networking v4, reasons v4) |
| test:integration | `pnpm test:integration` | **0** | 322 pass / 0 fail / 1 skipped (было 293; сброс welcome_test + миграции 001–011) |
| test:e2e | `pnpm test:e2e` | **0** | 24 passed (было 18; Playwright chromium, next dev :3111, welcome_e2e + axe a11y, теперь включая /me/connections) |
| build | `pnpm build` | **0** | Compiled successfully, 33 статические страницы |
| scan:secrets | `pnpm scan:secrets` | **0** | scan:secrets ok — no secrets found in tracked files |

Прогон гейтов: `pnpm gates` (scripts/run-gates.mjs) → `evidence/final-gates.json` + `evidence/final-gates.log`.
...
| B6 | активные сессии: список без токенов, текущая помечена | 200; ровно ключи id/created_at/last_seen_at/current; одна current и она первая; в теле нет слова token | сессий=31, current-первая=true, ключи строк только id/created_at/last_seen_at/current, токенов в теле нет | **PASS** | {"id":"ddb4bbe3-9a34-4ba4-b6ad-0cf9425dbe9f","created_at":"2026-09-15T10:15:22.627Z","last_seen_at":"2026-09-15T10:15:22.627Z","current":true} |
...
| D4 | QR SVG: 200 image/svg+xml | 200 + <svg | 200 image/svg+xml | **PASS** | HTTP 200 image/svg+xml; charset=utf-8 bytes=2020 |
...
| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| I1 | invite → claim-URL для approved-регистрации | 200 + claim_url/expires_at | 200 + claim_url=/claim/«token» (7 дней) | **PASS** | HTTP 200 {"ok":true,"claim_url":"/claim/«token»","expires_at":"2026-09-22T10:17:59.795Z"} |
...
| J10 | уведомления о решении подавляются по правилам (no_channel / consent_revoked) | decline → suppressed:no_channel; withdraw при активном binding без service_channel-согласия → suppressed:consent_revoked | decline=suppressed:no_channel (нет канала), withdraw=suppressed:consent_revoked (binding активен, согласие отозвано) | **PASS** | delivery_attempts: intro_declined:c62f27dc-7479-4e9b-93a7-ae015d277055:825d3145-dbb2-466c-b1e7-dae459eddc64 → suppressed/no_channel; intro_withdrawn:6e0640dc-1793-4a5d-aa82-0c97a6d8fb01:a19a7c8f-4d41-… |
| J11 | email-канал уведомлений: выбор канала и подавление | без привязки + email + согласие → email (не no_channel); с активной привязкой → telegram (email не используется); email без согласия → suppressed:consent_revoked; адрес не хранится в джобе | mailOnly=suppressed:channel_disabled (провайдера у экземпляра нет — подавление честное, канал всё равно email, не no_channel); mailBound=failed:tg_http_400 (telegram приоритетнее); mailNoConsent=suppressed:consent_revoked | **PASS** | delivery_attempts: intro_requested:6fede401-47c8-4a87-a1e1-2d6ffa0645db:7898322d-9f4c-4ec4-8990-97875026d2b6 → suppressed:channel_disabled; intro_requested:2f5de49c-341d-48df-8fab-41fd58a9f048:3b5e47d… |

## Режим K

| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| K1 | заметка владельцем: upsert + список | 200; заметка видна в своём списке | 200 + заметка в /api/me/notes | **PASS** | HTTP 200 {"ok":true,"note":{"other_profile_id":"«token»","note_text":"MATRIX- note about Delta","next_step":"ping","next_step_status":"proposed","updated_at":"2026-09-15T10:18:15.477Z"}} |
...
| P2 | health с x-health-details: отдаёт версию миграций | 200 + migration_version | 200 migrations=applied, migration_version=011 | **PASS** | HTTP 200 {"status":"ok","db":"up","migrations":"applied","migration_version":"011","worker":"down"} |
...
| S2 | статусы провайдеров: live только при наличии ключа, иначе disabled+причина | telegram/email по факту env, planned/disabled из реестра, только ИМЕНА переменных | telegram=live, email=disabled, ics=planned, linkedin=disabled(policy_restricted), vcard=live | **PASS** | HTTP 200 {"providers":[{"id":"telegram","kind":"channel","auth":"bot","capabilities":["send","receive","deeplink"],"direction":"both","status":"live","reason_code":null,"missing_env":[],"env":["TELEGR… |
| S3 | GET /api/events/<slug>/ics — публичный календарный файл | 200 text/calendar, attachment, UID, UTC, без комнатной ссылки | 200, text/calendar, attachment, DTSTART UTC, UID по id события, комнатная ссылка отсутствует | **PASS** | content-type=text/calendar; charset=utf-8 disposition=attachment; filename="welcome-matrix-ics-ea55db3.ics" bytes=387 |
...
| S5 | страница /e/<slug>: кнопки календаря и диплинки шэринга | есть ссылка на .ics, Google Calendar и 4 сети с rel=noopener noreferrer | .ics + Google Calendar + 4 сети, 5 ссылок с rel="noopener noreferrer" | **PASS** | HTTP 200, 12673 bytes |
...
| S9 | /me/connections: закрыт без сессии, с сессией показывает провайдеров без секретов | signed-out → redirect /login; signed-in → 200 + все id + только ИМЕНА переменных | signed-out 307 → /login; signed-in 200 с 15 карточками и именами переменных | **PASS** | signed-out: 307 /login \| signed-in: HTTP 200, 88534 bytes |
| S10 | публичная карточка: og:url, «Добавить в контакты» и диплинки | og:url карточки, ссылка на vCard без сессии, 4 сети | 200: og:url + vCard + 4 сети, анонимная выгрузка .vcf работает, приватных целей нет | **PASS** | HTTP 200, 17740 bytes \| vCard HTTP 200 |

## Отклонения от ожиданий задания

- **С первого раза прогон не состоялся (39 FAIL) — причина в конфигурации раннера, а не в продукте.** Локальный `next start` был запущен с неполным ADR-0009 allowlist: в списке не было адреса владельца (`nberezniker@gmail.com`) и `matrix-claim@welcome.test`. Каждый вход этих аккаунтов получал 503 `email_channel_disabled` (в production без `RESEND_API_KEY` код не отдаётся не-allowlist'нутым адресам), раннер повторял запросы, те исчерпали персональное окно «3 кода / 15 мин» — и все проверки, которым нужна сессия владельца или claim, упали каскадом. Второй источник каскада: при пустом `campaignId` URL вида `/api/organizer/campaigns//approve` отдаёт 308 (нормализация слеша), что читалось как «ожидали 403, получили 308». После добавления allowlist и переменных Vertex-обогащения (`ENRICHMENT_PROVIDER=vertex-gemini`, `GCP_PROJECT_ID`, `GCP_SA_JSON_B64`) прогон дал 122 PASS / 0 FAIL. Цифры в отчёте — из чистого прогона; первая попытка упомянута здесь, потому что это грабли, на которые упрётся следующий оператор.
- S2: на локальном экземпляре `email` = `disabled` / `not_configured` (в `.env.deploy.secrets` нет `RESEND_API_KEY`) — проверяется инвариант «live ⟺ переменная есть», а не конкретное значение.
- S8: чеки режимов идут на отдельном MATRIX-событии, а не на событии A: там к этому моменту уже есть знакомства от предыдущих режимов, и рекомендер (корректно) скрывает пары в кулдауне — иначе порядок режимов влиял бы на результат.
- `excluded_reason` трактуется как ответ на вопрос «почему список пуст» (`no_candidates` / `gate_not_met` / `no_shared_topic`; `null`, когда список непустой): в ТЗ поле названо, но не описано, трактовка зафиксирована в коде (`RecommendationsResult`).
- Цели (`goals`) в POST /api/me/profile ведут себя как остальные поля профиля: отсутствие поля = пустой список (очистка). Зафиксировано тестами и helper'ом матрицы; клиент, не присылающий `goals`, целей не сохранит.
- Миграция 011 применена к прод-БД **до** прогона (без неё ни goals, ни рекомендации не работают) — тем же порядком, что предписан в deploy-notes: сначала миграция, потом код.
...
Создано за прогон: 17 аккаунтов, 16 профилей, 7 событий, 2 организаторов, 2 кампаний. Все имена — с префиксом `MATRIX-` (display_name, названия событий и организаторов, body кампаний), слаги — `matrix-…`.
