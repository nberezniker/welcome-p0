...
- **Прогон:** 2026-09-20T22:45:58.546Z → 2026-09-20T22:49:52.811Z
...
| B6 | активные сессии: список без токенов, текущая помечена | 200; ровно ключи id/created_at/last_seen_at/current; одна current и она первая; в теле нет слова token | сессий=46, current-первая=true, ключи строк только id/created_at/last_seen_at/current, токенов в теле нет | **PASS** | {"id":"d542e9ee-6e51-431d-8e26-c44b5a7b7dbc","created_at":"2026-09-20T22:46:03.128Z","last_seen_at":"2026-09-20T22:46:03.128Z","current":true} |
...
| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| I1 | invite → claim-URL для approved-регистрации | 200 + claim_url/expires_at | 200 + claim_url=/claim/«token» (7 дней) | **PASS** | HTTP 200 {"ok":true,"claim_url":"/claim/«token»","expires_at":"2026-09-27T22:49:10.597Z"} |
...
| J10 | уведомления о решении подавляются по правилам (no_channel / consent_revoked) | decline → suppressed:no_channel; withdraw при активном binding без service_channel-согласия → suppressed:consent_revoked | decline=suppressed:no_channel (нет канала), withdraw=suppressed:consent_revoked (binding активен, согласие отозвано) | **PASS** | delivery_attempts: intro_declined:8f6af078-50bc-461a-bcfb-45f9b6729c7e:2070c656-105a-4bbc-9fc4-f17c75126a63 → suppressed/no_channel; intro_withdrawn:73c6f47d-b1d2-42d4-8131-5a5ed46c0c5a:c3bba021-1198-… |
| J11 | email-канал уведомлений: выбор канала и подавление | без привязки + email + согласие → email (не no_channel); с активной привязкой → telegram (email не используется); email без согласия → suppressed:consent_revoked; адрес не хранится в джобе | mailOnly=sent:email (провайдера у экземпляра нет — подавление честное, канал всё равно email, не no_channel); mailBound=failed:tg_http_400 (telegram приоритетнее); mailNoConsent=suppressed:consent_revoked | **PASS** | delivery_attempts: intro_requested:a83fd93d-8f0e-49af-b859-1fdc15dc43f6:83c7e61d-5ffd-4fdf-864e-120a21e6bbf3 → sent:email; intro_requested:f4ce70ba-e0ba-46dd-8db5-b86a79559e4a:b389ff71-acd8-4ef3-a588-… |

## Режим K

| ID | Проверка | Ожидание | Факт | Статус | Доказательство |
|---|---|---|---|---|---|
| K1 | заметка владельцем: upsert + список | 200; заметка видна в своём списке | 200 + заметка в /api/me/notes | **PASS** | HTTP 200 {"ok":true,"note":{"other_profile_id":"«token»","note_text":"MATRIX- note about Delta","next_step":"ping","next_step_status":"proposed","updated_at":"2026-09-20T22:49:18.412Z"}} |
...
| S5 | страница /e/<slug>: кнопки календаря и диплинки шэринга | есть ссылка на .ics, Google Calendar и 4 сети с rel=noopener noreferrer | .ics + Google Calendar + 4 сети, 5 ссылок с rel="noopener noreferrer" | **PASS** | HTTP 200, 16316 bytes |
...
| S9 | /me/connections: закрыт без сессии, с сессией показывает провайдеров без секретов | signed-out → redirect /login; signed-in → 200 + все id + только ИМЕНА переменных | signed-out 307 → /login; signed-in 200 с 15 карточками, панелью импорта и именами переменных | **PASS** | signed-out: 307 /login \| signed-in: HTTP 200, 103363 bytes |
| S10 | публичная карточка: og:url, «Добавить в контакты» и диплинки | og:url карточки, ссылка на vCard без сессии, 4 сети | 200: og:url + vCard + 4 сети, анонимная выгрузка .vcf работает, приватных целей нет | **PASS** | HTTP 200, 19971 bytes \| vCard HTTP 200 |
| S11 | POST /api/me/contacts/import: fixture-сосед находится по адресу из .vcf | 200, matched по slug, в теле нет ни одного email, 5/час | 200: scanned=2, matched=1 (MATRIX-Bravo / matrix-bravo-5b9c7de6d3cfbd35), unmatched=1, ни одного email в ответе | **PASS** | HTTP 200 {"ok":true,"scanned":2,"matched_count":1,"unmatched_count":1} \| X-RateLimit-Limit=5 |
