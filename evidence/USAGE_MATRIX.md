...
|---|---|---|---|---|
| WP1 | Email как второй канал доставки (выбор канала на момент отправки, ADR 0011) | `4e66f47` | src/infra/recipient-channel.ts (чистое правило выбора + резолвер адреса из claimed-регистрации), src/domain/service-notices.ts (тексты писем из KIND, без имён/контактов), src/infra/worker.ts (выбор канала + email-диспатч), event_id в payload уведомлений, ADR 0011. | unit: service-notices (7), recipient-channel (8); integration: email-channel (7 — email/telegram-приоритет/consent_revoked/channel_disabled/без утечки адреса в логи и payload); живой чек J11. |
| WP2 | Воронка организатора: эндпоинт аналитики + блок с процентами и SVG-графиком | `6936787` | src/domain/event-analytics.ts (10 агрегатов + by_day 30 дней), GET /api/organizer/events/[eventId]/analytics (owner/admin, no-store), FunnelPanel (шаги с процентом от предыдущего, 30-дневный SVG без внешних либ), i18n EN/RU/ES. | unit: event-analytics (5); integration: event-analytics (2 — агрегаты и 403/401); e2e: organizer-funnel; живые чеки N13/N14. |
| WP3 | Сегменты аудитории кампаний по осям таксономии v3 | `8feebe8` | audience_filter в src/domain/campaigns.ts (валидация по каталогу, пусто = без ограничения), фильтр в currentEligibleAudience/approve/send, аудитория-эндпоинт с эхо фильтра и query-override, SegmentPicker в форме кампании, i18n EN/RU/ES. | unit: campaign-segments (10); integration: campaign-segments (3 — отбор, preview==send, снятие согласия); e2e: organizer-campaign-segment; живые чеки N10–N12. |
...
| test:unit | `pnpm test:unit` | **0** | 355 pass / 0 fail (node --test + tsx, 355 tests) |
| test:integration | `pnpm test:integration` | **0** | 261 pass / 0 fail / 1 skipped (сброс welcome_test + миграции 001–009) |
...
| N13 | воронка события: агрегаты без PII для владельца | 200 со всеми 10 метриками и by_day из 30 дней | members_active=8, intros_requested=6, mutual=1, declined=1, by_day=30 дн. | **PASS** | {"registrations_total":6,"registrations_claimed":4,"members_active":8,"members_directory_visible":7,"intros_requested":6,"intros_mutual":1,"intros_declined":1,"reveals_total":0,"notes_created":1,"atte… |
